...
class ViterbiNet:
    ...
    def __call__(self,
        ...
        for k in range(num_frames):
            ...
            # updates viterbi parameters trellis diagram cost and checks if the bounding box is possible

        best_track = self.tracks[int(torch.argmax(self.costs).item())]
        if cheat_state is not None:
            del best_track[0]  # TODO maybe delete best_track[-1]
        return best_track
    ...
    def Viterbi_step(self, z_k):
        ...
        mask = vit_utils.beam_mask(LogLikelihood, self.beta)  # Applying an adaptive threshold to further narrow down
        # the list of candidate transitions
        ...
        for i in range(num_candidates):
            ...
            costs[i] = LogLikelihood[r, d] + step_cost  # log f_x_k(z_k;params) +C_k-1(0_k-1(x_k))+log p(x_k|0_k-1(x_k))
    ...
    def find_prev(self, current):
        ...
        for i in range(len(self.tracks)):
            ...
            costs[i] = cost + self.mm.step_cost(current, (r, d))  # C_k-1(0_k-1(x_k)) + log p(x_k|0_k-1(x_k))
        prev_idx = torch.argmax(costs).item()  # The most likely path leading to current state
    ...
    def Weighted_Origin(self):
        ...
        # The estimated center of mass is taken as the center of the search region in the next time instance:
        weights = torch.softmax(self.costs, dim=0)
        r_avg = torch.sum(ranges * weights, dim=0)  # Average of all range values in self.ranges from previous z_k bbox
        v_avg = torch.sum(velocities * weights, dim=0)  # Average of all doppler values currently in self.tracks
