...
class Track_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.num_samples):
            # r, d = self.env.val2idx(r_val[i][0], d_val[i][0])  # Range and doppler of the 20SNR target
            r, d = self.env.val2idx(r_val[i][1], d_val[i][1])  # Range and doppler of the 10SNR target
            label.append((r, d))
        return data, label
